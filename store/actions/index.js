...
export const registerNewUser =
  (sendData, toast, reset, navigate, setLoader) => async (dispatch) => {
    try {
      //now ui wil be in loading state
      setLoader(true);
...
export const logOutUser = (navigate) => (dispatch) => {
  //when logout action action is triggered than we will will do this
  dispatch({ type: "LOG_OUT" });

  //WHENWE REMOVE ITEM WE WILL REMOVE AUTH
...
export const addUpdateUserAddress =
  ...
  async (dispatch, getState) => {
    ...
    try {
      ...
      //when address is updated we will refresh the page automaticaly to load the values from the redux stroe when the value is updated in the server so we will load from the server
      ...
      console.log(error);
      toast.error(error?.response?.data?.message || "Internal Server Error");
      //it is managed in the error reducer
...
export const deleteUserAddress =
  (toast, addressId, setOpenDeleteModal) => async (dispatch, getState) => {
    try {
      ...
      dispatch({ type: "IS_SUCCESS" });
      dispatch(getUserAddresses());
      //clearing the selected option from redux
...
//calling paymentmethod in the redux which will
...
//so we are fetching values from analyticAction and adding that to the store
